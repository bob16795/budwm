...
drawbar(Monitor *m)
{
  ...
  if (p) { /* status is only drawn on selected monitor */
    sw = TEXTW(p) - lrpad / 2 + 2; /* 2px right padding */
    drw_text(drw, m->ww - sw - stw, 0, sw, bh, lrpad / 2 - 2, p, 0);
  }
  resizebarwin(m);
  ...
  if (m->sel) {
    ...
    drw_text(drw, x, 0, (m->ww - stw - x - sw), bh, mid, m->sel->name, 0);
  }else{
    drw_rect(drw, x, 0, m->ww - stw - x - sw, bh, 1, 1);
  }
