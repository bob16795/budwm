...
drawbar(Monitor *m)
{
  ...
  x = 0;
  if (l) { /* status is only drawn on selected monitor */
    sw = TEXTW(r) - lrpad / 2 + 2; /* 2px right padding */
    drw_text(drw, m->ww - sw - stw, 0, sw, bh, lrpad / 2 - 2, r, 0);
    w = TEXTW(m->ltsymbol);
...
void setcontainer(const Arg *arg) {
  unsigned int f;
  Client *c;
...
setfullscreen(Client *c, int fullscreen)
{
  if (fullscreen && !c->isfullscreen) {
    ...
    c->bw = 0;
    c->isfloating = 1;
    resizeclient(c, c->mon->mx, c->mon->my, c->mon->mw - 2 * c->oldbw, c->mon->mh - 2 * c->oldbw );
    XRaiseWindow(dpy, c->framewin);
