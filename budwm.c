...
static void drawframe(Client* c);
...
static void incvsplit(const Arg *arg);
...
static void setabsplit(const Arg *arg);
static void setacsplit(const Arg *arg);
static void setbdsplit(const Arg *arg);
static void setcontainer(const Arg *arg);
...
drawbar(Monitor *m)
{
  ...
  while(t) {
    ...
    if (t[0] == '\xf2') {
      ...
      sw += w;
      drw_text(drw, m->ww - sw - stw, 0, w, bh, 0, t, 0);
    }
    t = strtok(NULL, "\xf0");
  }
  drw_setscheme(drw, scheme[SchemeNorm]);
    ...
    if (frametabs & (mode == 1)& !c->isfloating) {
      ...
      for (cb = c->mon->clients; cb; cb = cb->next){
        if (ISVISIBLE(cb) && cb->container == c->container && !cb->isfloating){
          ...
          cur ++;
          if (frameicons){
            drw_text(drw, cur * w  - TEXTW(icon) - TEXTW(" "), 0, TEXTW(icon), bh - 2, 0, icon, 0);
          }
        }
      }
    } else {
      ...
      if (frameicons){
        drw_text(drw, c->w - TEXTW(icon) - TEXTW(" "), 0, TEXTW(icon), bh - 2, 0, icon, 0);
      }
...
focus(Client *c)
{
  ...
  if (c) {
    ...
    attachstack(c);
    grabbuttons(c, 1);
    XSetWindowBorder(dpy, c->framewin, scheme[SchemeSel][ColBorder].pixel);
    setfocus(c);
  } else {
...
grabbuttons(Client *c, int focused)
{
  updatenumlockmask();
  {
    ...
    XUngrabButton(dpy, AnyButton, AnyModifier, c->framewin);
    if (!focused)
...
void
inchsplit(const Arg *arg){
  if (selmon->sel->container == 1 || selmon->sel->container == 3){
    if (arg){
      if (arg->i != 0)
        acsplit += arg->i;
    }
...
resizemouse(const Arg *arg)
{
  ...
  if (XGrabPointer(dpy, root, False, MOUSEMASK, GrabModeAsync, GrabModeAsync,
    ...
    return;
  XWarpPointer(dpy, None, c->framewin, 0, 0, 0, 0, c->w + c->bw - 1, c->h + c->bw - 1);
  do {
    XMaskEvent(dpy, MOUSEMASK|ExposureMask|SubstructureRedirectMask, &ev);
    switch(ev.type) {
    case ConfigureRequest:
    ...
    case MotionNotify:
      ...
      nw = MAX(ev.xmotion.x - ocx - 2 * c->bw + 1, 1+bh);
      nh = MAX(ev.xmotion.y - ocy - 2 * c->bw + 1, 1+bh);
...
restack(Monitor *m)
{
  ...
  if (m->sel->isfloating || !m->lt[m->sellt]->arrange)
    XRaiseWindow(dpy, m->sel->framewin);
  if (m->lt[m->sellt]->arrange) {
    wc.stack_mode = Below;
    wc.sibling = m->barwin;
    for (c = m->stack; c; c = c->snext)
      if (!c->isfloating && ISVISIBLE(c)) {
        XConfigureWindow(dpy, c->framewin, CWSibling|CWStackMode, &wc);
...
showhide(Client *c)
{
  ...
  if (ISVISIBLE(c)) {
    ...
    XMoveWindow(dpy, c->framewin, c->x, c->y);
    ...
    showhide(c->snext);
    XMoveWindow(dpy, c->framewin, WIDTH(c) * -2, c->y);
  }
}
...
view(const Arg *arg)
{
  ...
  for (Monitor* m = mons; m; m = m->next){
    if (arg->ui & TAGMASK) {
      m->tagset[seltags] = arg->ui & TAGMASK;
      ...
      m->pertag->prevtag = selmon->pertag->curtag;
...
main(int argc, char *argv[])
{
  ...
  if (!(xcon = XGetXCBConnection(dpy)))
		die("budwm: cannot get xcb connection\n");
  checkotherwm();
  XrmInitialize();
  loadxrdb();
  setup();
